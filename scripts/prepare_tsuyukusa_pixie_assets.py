...
SOURCE = Path(
    ...
    "ig_072a9ec9062cdd78016a364ed5fe6c8191896d0084516d3859.png"
)
ASSET_ROOT = ROOT / "assets" / "otomon" / "tsuyukusa_pixie"

PATHS = {
    "original": ASSET_ROOT / "source" / "tsuyukusa_pixie_original.png",
    1024: ASSET_ROOT / "1024" / "tsuyukusa_pixie_1024.png",
    256: ASSET_ROOT / "256" / "tsuyukusa_pixie_256.png",
    64: ASSET_ROOT / "64" / "tsuyukusa_pixie_64.png",
}
...
def connected_white_background(rgb):
    ...
    near_white = (
        ...
        & (arr[:, :, 1] >= 244)
        & (arr[:, :, 2] >= 244)
        & ((arr[:, :, 0].astype(int) - arr[:, :, 2].astype(int)) < 12)
    )
...
def remove_white_background(image):
    ...
    # Remove a little of the almost-white edge fringe without touching isolated dew highlights.
    edge = (
        (arr[:, :, 3] > 0)
        & (arr[:, :, 0] >= 250)
        & (arr[:, :, 1] >= 250)
    ...
    arr[edge, 3] = np.minimum(arr[edge, 3], 80)
...
def main():
    ...
    PATHS[1024].parent.mkdir(parents=True, exist_ok=True)
