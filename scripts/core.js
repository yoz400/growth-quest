...
function buildWorldMap() {
    ...
    return `
      <g class="wm-area">
        <circle cx="${a.map.x}" cy="${a.map.y}" r="26"
                fill="${a.palette.base}" fill-opacity=".76"
                stroke="${a.accent}" stroke-width="2"/>
  ...
  // ③ 大陸そのものは1枚の絵（assets/worldmap.webp）。
  //    以前はSVGで山や川を手描きしていたが、絵に置き換えた。
  //    ⚠️ 絵と areas.js の map 座標は対で管理する。絵を差し替えたら座標も置き直すこと。
  svg.innerHTML = `
    <defs>
      <!-- 雲は「隠す」ためではなく「かすませる」ためのもの。
           大陸の姿（地形・海岸線）は透けて見えてほしい。
    ...
    <rect class="wm-sea" x="0" y="0" width="680" height="383"/>
    <image class="wm-art" href="assets/worldmap.webp" x="0" y="0" width="680" height="383"/>
    <rect class="wm-veil" x="0" y="0" width="680" height="383"/>
