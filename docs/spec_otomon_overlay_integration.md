...
> ✅ **レビュー完了（2026-07-15 クロ・?v=guild-82）**: C-2（誕生演出）を精査＋実機で検証し合格。
> - **差分**: DEFSに `otomon-birth-overlay` 登録／showBirth/closeBirth を Overlay.open/close 化
>   （injectBirth＋内容セットは従来どおり open 前）／CSS共通selectorに追加
> - **「図鑑で見る」フロー**: ボタンは `() => { closeBirth(); openPanel(); }`＝**閉じてから開く**。
>   実機で誕生演出クローズ→図鑑オープン、スタックは図鑑1枚のみ（誕生が下に残らない）を確認
> - **実機**: 自動表示で top=birth・背景inert・visible／閉じるボタン・背景タップ・**ESC**で閉じる／
>   コンソールエラーゼロ
...
> ✅ **レビュー完了（2026-07-15 クロ・?v=guild-82）**: C-3（bond昇格演出）を精査＋実機で検証し合格。
> - **差分**: DEFSに `otomon-bondup-overlay` 登録／showBondUp/closeBondUp を Overlay.open/close 化
>   （injectBondUp＋内容セットは従来どおり open 前）／CSS共通selectorに追加。閉じるボタン・
>   背景タップ(e.target===ov)配線は closeBondUp→Overlay.close 経由・独自ESCハンドラなし
> - **告(Koku)との重なり＝C-0で懸念した最重要点**: bond昇格は告の上に自動で出るため、
>   同一機構の showBirth を告の上に出してスタック挙動を検証（bondupは公開APIでないため代表検証）。
>   結果=**告が開いたまま上にオトモン画面が正しくスタック**（上=操作可/aria-hidden:false、
>   下の告=inert・aria-hidden:true・active維持）→上を閉じると**告に戻りinert解除・告はactive継続**。
>   bondupもまったく同じ Overlay.open/close 経路なので同挙動
> - **実機**: bondup単体も top=bondup・背景inert・visible で正常。addBond/bondTier公開API健在／
>   全体スモーク（設定・図鑑・誕生再演出）OK・コンソールエラーゼロ
>
> ## ✅ Phase C 完了（C-1/C-2/C-3 全合格・?v=guild-82・本番デプロイ済み）
> オトモンの3オーバーレイすべてが OverlayManager 管理下に統一された。弱点4（otomonだけ
> 独自規範）は解消。全オーバーレイで ESC・背景inert・フォーカストラップ・スタック管理・
> スマホ戻るボタン対応が一元化。**otomon.js の独自 classList 開閉は根絶**（残る classList は無し）。
