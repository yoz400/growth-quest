...
- [ ] 集中を終えても**サイコロが自動で回らない**。告にサイコロの演出が出ない
- [ ] 告に「🎲 すごろくを振れます（あと N 回）」が1行出る（0枚のときは出ない）
- [ ] すごろくを開くと最上部に「🎲 振る」ボタンがあり、残り枚数が出ている
- [ ] 押すと1枚減り、サイコロ→コマの歩行→マスのイベント→報酬が**従来どおり**動く
- [ ] XPは**振ったときに**入る（セッション完了時には入らない）
- [ ] 0枚のときボタンは押せず、「集中を終えると振れます」と出る
- [ ] 歩行アニメ中は押せない（連打しても二重に進まない）
- [ ] チケットは11枚目から古いものが捨てられ、最大10枚で頭打ち
- [ ] ヘッダーの🎲ボタンに枚数バッジが出る（0枚なら消える）。**ボタンは増えていない**
- [ ] `python3 tools/check_load_order.py` が通る
- [ ] コンソールエラーゼロ
- [ ] `bash tools/bump_version.sh` を実行した
