...
- [x] 「きょう」層にタブが出て、押すと中身が切り替わる
- [x] 手帳OFFの人に「予定」タブが出ない／打刻未解放の人に「打刻」タブが出ない
- [x] タブが1つだけになる人（新規ユーザー）には**タブバーごと出ない**
- [x] 予定を1件追加すると「予定」タブが**その場で**出る（リロード不要）
- [x] 「予定」タブを開いてリロード → **予定タブのまま**
- [x] 日付をまたぐと**クエストタブに戻る**（検証は §6 の手順4で日付を偽装する）
- [x] 昨日見ていたタブが今日は出せない場合、クエストにフォールバックし、**エラーを出さない**
- [x] タブのコントラストと文字サイズを**測って数値を報告**（4.5:1 以上 / 10px 以上）
- [x] **オトモン3枚がタブの外・「きょう」層の下端**にあり、どのタブを開いても見える
- [x] **オトモンの並びが 🤝お供 → ⚡クエスト → 🥚卵 のまま**（逆になっていない）
- [x] オトモン未解放の人に、余分な空白が残っていない
- [x] **タブを切り替えたあとに予定を1件足しても、開いていたタブが戻らない**
      （系統①と②の分離ができているか。§5-2 の最頻出バグ）
- [x] `gq_home_tab` が台帳に載っている
- [x] **`sw.js` の `PRECACHE_URLS` に `home-layers.js` が入り、`CACHE_NAME` が上がっている**
- [x] **オフラインで起動する**（DevTools の Network を Offline にしてリロード）
- [x] `index.html` の新しい `<script>` に `?v=guild-N` が付いている
- [x] ~~STARTの上端を測って報告（**縮むはず**）~~ → **この基準は間違いだった**。
      「きょう」層はタイマーより**下**にあるので、畳んでも START は動かない（実測で確認・§9）
- [x] `python3 tools/check_load_order.py` 通過（**新ファイルを足すので必ず実行**）
...
### ✅ L-3 完了（2026-08-06 Codexが実装・クロがレビュー）`?v=guild-150`

**一発合格。差し戻しゼロ。** 三層化はこれで完了。

| 検証項目 | 結果 |
|---|---|
| 新規ユーザー（クエストのみ） | ✅ タブバーごと出ない |
| 予定を1件追加 | ✅ リロード無しで「予定」タブがその場に出る |
| タブ切り替え | ✅ 選んだ中身だけが出る |
| **タブを開いたまま予定を操作** | ✅ **タブが戻らない**（系統①②の分離ができている） |
| リロード | ✅ 「予定」タブのまま |
| 日付またぎ（昨日=打刻で偽装） | ✅ クエストへ戻る・エラーなし |
| 保存タブが今日は出せない場合 | ✅ 黙ってクエストへ・保存も更新される |
| オトモン3枚 | ✅ タブの外・層の下端・どのタブでも見える |
| オトモンの並び | ✅ 🤝お供 → ⚡クエスト → 🥚卵（`prepend` で維持） |
| オトモン未解放の人 | ✅ 余分な空白なし（`:empty` ＋ `refreshHome()` の `hidden`） |
| `sw.js` | ✅ `gq-cache-v15` に11本。`home-layers.js` が実際にキャッシュされている |
| タブの文字 / コントラスト / タップ領域 | ✅ 13.12px / 16.21:1・5.85:1 / 96x35・70x35 |
| 320px で3タブ | ✅ 1行に収まる（96+70+67px）・横スクロールなし |
| `check_load_order.py` / コンソールエラー | ✅ 通過 / ゼロ |

**Codexが仕様を超えて足した1点**（採用）: `refreshHome()` で
「3枚とも `display:none` なら常設枠ごと `hidden`」にしている。
`:empty` だけでは**注入済みだが全部非表示**の人に10pxの空白が残る。よく気づいた。

**「きょう」層は 654px → 539px に縮んだ**（クエスト＋予定＋オトモン3枚が出ている状態）。

> 🚨 **私が書いた受け入れ基準が1つ間違っていた。**
> 「タブに畳めば START が縮むはず」と書いたが、**START はタイマーカードの中にあり、
> タイマーは「いま」層＝きょう層より上**。下の層を畳んでも上の要素は動かない。
> タブバーを消して測っても START は 581px のまま変わらなかった（直接確認）。
>
> 実測値が L-2 の 573px→581px、567px→597px と**増えて**見えたのは、
> **ヘッダーの名言がランダムで、長いと3行になり30px高くなる**ため。
> 名言を短い文字列に差し替えると、同じ状態で 597px → **567px**（＝L-2の実測値と一致）。
>
> 💡 **学び: 数値の基準を書くときは、その数値が何で決まるかを先に確かめる。**
> 「縮むはず」に根拠は無く、レイアウトの上下関係を見れば動かないと分かった。
> H-2 の「44px」と同じ失敗（根拠を確かめずに数字を要件にした）を繰り返している。
