...
| B ✅済 | **イベント通知の導入**: `GQ.emit('session:complete', {mins})` を作り、各機能は購読して自分の再描画だけ行う | 弱点2の根治。「100機能」への本丸。新機能は emit を1行も触らず購読だけで参加できる | 済（2026-07-13、3イベント＋購読移行。spec_event_bus.md） |
| C ✅済 | otomonモーダルのOverlayManager統合 | 弱点4解消 | 済（2026-07-15、3画面統合。spec_otomon_overlay_integration.md） |
| D ✅済 | 各ファイルをIIFE（即時関数）で包み、公開APIだけ`window.*`に載せる | 弱点1の根治 | 済（2026-07-15、全8ファイルIIFE化・?v=guild-90。spec_iife_namespacing.md。※共有状態の名前空間移設はEに残す） |
