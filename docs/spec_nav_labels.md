...
- [x] 全ヘッダーボタンの絵文字の下に小さなラベルが常時表示される（スマホ含む）
- [x] ロック中の機能はシルエット＋「？？？」で見え、空白のすき間がない
- [x] ロック中ボタンをタップすると機能は開かず、解放条件のヒントトーストが出る
- [x] 解放済みになるとラベル・見た目・タップ動作が通常に戻る（既存の🔓トースト＆光る演出はそのまま動く）
- [x] 全ヘッダーボタンに `aria-label` が付き、ロック中は「ロック中」と分かる読み上げになる
- [x] 画面幅320pxでもヘッダーが折り返し・はみ出ししない
- [x] オトモン🥚ボタン（otomon.js が後から注入）にもラベルが付く
- [x] 既存機能（各ボタンで開く画面、導きの妖精のハイライト等）が壊れていない

> ✅ **レビュー完了（2026-07-04 クロ）**：Codex実装（4コミット）をブラウザ実機で検証し合格。
> ロックのシルエット・？？？ラベル・ヒントトースト・クリックガード・解放時の復帰
> （ラベル/aria/グロー/クリック動作）・オトモンボタン・aria全付与を確認。
> **レビューで発見した不具合1件はクロが修正**：仕様の受け入れ基準が「320px」だけを
> 指定していたため、375〜420px（iPhone標準幅）で8ボタンが収まらず⚙設定が画面外に
> 出ていた。520px以下はボタンを均等幅で伸縮させるCSSを追加して解消（?v=guild-50）。
> 320px / 375px / 497px で、はみ出しゼロを再検証済み。**STEP2完了。**
> 教訓: 幅系の受け入れ基準は「最小幅」でなく「320〜520pxの全域」で書くこと。
