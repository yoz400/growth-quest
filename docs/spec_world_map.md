...
> 🚨 **2026-08-02 追記：この §6.2 はまだ不十分だった。もう一度訂正する。**
> `worldMaxCell()` を**雲にも現在地表示にも兼用させたのが誤り**だった。
> 2周目に入るとこの2つは食い違うので、**必ず別々の値にすること**。
>
> ```text
> ステージ2・pos=155 のとき
>   到達済み  = 100  → 雲は全部晴れている（正しい）
>   現在地    = 55   → 2周目の55マス目＝鏡面の湖（すごろく画面もこう出る）
>
> 兼用していたときの症状:
>   すごろく「マス 55 / 100 ・ 鏡面の湖」
>   大陸図  「マス 100 / 100 ・ 暁の頂 に到達！」   ← 食い違う
> ```
>
> ```javascript
...
> ```
>
> **使い分け**: `worldMaxCell()` → 雲・通った道・`markSeen`／
> `worldCurrentCell()` → ▲・見出しのマス番号・足元のエリア名と残りマス数。
...
### 🐛 修正（2026-08-02）：2周目で「いる場所」がズレるバグ

**症状**（ヨージが実機で発見）: すごろくは「マス 55 / 100 ・ 鏡面の湖」なのに、
大陸図を開くと「マス 100 / 100 ・ 暁の頂 に到達！」と出る。▲も暁の頂に立つ。

**原因**: 仕様書 §6.2 で入れた `worldMaxCell()`（2周目は100で頭打ち）を、
**雲の計算にも現在地表示にも兼用していた**。ステージ1では両者が一致するので
Phase 3 のレビューでは踏めなかった。ステージ2に入って初めて食い違う。

```text
pos = 155（2周目・55マス目）

  雲に必要な値      = 100  「1周終えたので全部晴れている」
  現在地に必要な値  = 55   「いまは鏡面の湖にいる」
        ↑ この2つを1つの変数で兼ねていた
```

**修正**: `worldCurrentCell()` を足して用途を分けた（§6.2 の追記を参照）。

| 用途 | 使う値 |
|---|---|
| 雲の濃さ／通った道／`markSeen` | `worldMaxCell()`（到達済みの最大） |
| ▲の位置／見出しのマス番号／足元のエリア名と残りマス数 | `worldCurrentCell()`（いまいる場所） |

あわせて、2周目以降は見出しに「2周目」と出すようにした。全部晴れている理由が
分かるようにするため。
...
| `pos` | すごろく | 大陸図 | 雲 |
|---|---|---|---|
| 37（1周目） | マス37・忘れられた遺跡 | マス37・忘れられた遺跡 7/10 | `[0,0,0,0.3,1,…]` |
| 101（2周目の頭） | マス1・はじまりの草原 | マス1・**2周目**・はじまりの草原 1/10 | 全部晴れ |
| 155（2周目） | マス55・鏡面の湖 | マス55・**2周目**・鏡面の湖 5/10 | 全部晴れ |

1周目の見え方は変わっていない（回帰なし）。コンソールエラーゼロ。

> 💡 **学び**: 「同じ数字に見える2つの概念」を1つの変数で兼ねると、
> 普段は一致しているので気づけず、境界（ここでは2周目）で初めて壊れる。
> 受け入れ基準に「ステージ2で雲が戻らないこと」は入れていたが、
> **「ステージ2で現在地が正しいこと」は入れていなかった**。
> 次に周回のある機能を作るときは、**1周目と2周目の両方を必ず確認する**。
