...
S3-1:
- [x] 繰り返し予定の🗑でインライン3択が出て「この日だけスキップ」でその日だけ消える
- [x] スキップした予定は翌日以降（次の該当日）には出る。カレンダーのバッジ数も追従
- [x] 「すべての回を削除」は従来どおり全削除。単発予定は従来どおり即削除
- [x] 予定のテキストをタップ→フォームに値が入り「保存」で上書き（id/done/skip維持）
- [x] 既存の gq_planner データ（skipDates無し）がそのまま動く

S3-2:
- [x] 今日の予定がある日はホームにカードが出て、チェックで完了できる
- [x] 0件の日はカードが出ない
- [x] 日ポップアップで予定を追加/削除するとホームカードが即追従する
- [x] 「＋」で今日の日ポップアップが開く
- [x] day:changed 購読は setTimeout 遅延つき（spec_event_bus.md の注意を遵守）

S3-3:
- [x] 定刻でOS通知/バナーに加えてチャイムが鳴る（設定OFFで鳴らない）
- [x] トーストの「10分後にもう一度」で10分後に再通知（同じ予定は1回まで）

共通:
- [x] 新しい localStorage キーを作っていない（gq_settings/gq_planner の中身拡張のみ）
- [x] 新関数はIIFE内。window公開が必要になったら止まって報告
- [x] クラウド同期（syncPlannerToCloud）は無変更で従来どおり動く
- [x] スマホ幅320〜375pxで崩れなし・コンソールエラーゼロ・bump_version.sh 実行済み


> ✅ **レビュー完了（2026-07-16 クロ・?v=guild-97）**: Codex実装3コミット
> （baf8a31 S3-1／0e63bc2 S3-2／8a79fd5 S3-3）を精査＋実機で検証し**全項目合格**。
> - **S3-1（2段階レビュー）**: コミット直後に検証済み。スキップ3択・skipDates=当日のみ・
>   単発即削除・編集モード（値読込→保存でid/kind維持→フォーム復帰）・**ポップアップを開く
>   たびresetPlannerForm＝編集状態の引きずり事故予防**まで実装。合格
> - **S3-2**: 未コミット時点でレビュー→クロ指摘（bump未実行）が反映されguild-96でコミット。
>   ホームカード時刻順3件・チェック永続化・0件非表示・＋→今日ポップアップ（showDayPopupの
>   第2引数cellElは未使用と裏取り済み）・savePlanner→renderHomePlanner連動・
>   **day:changed購読はsetTimeout遅延つき＝イベントバス初の実購読が仕様どおり**。合格
> - **S3-3**: DEFAULT_SETTINGSにreminderSound:true／設定トグル（OFF無音・ONでプレビュー
>   チャイム・保存確認）／fireReminderでplayChime（timer.js公開関数をwindow経由で解決）／
>   スヌーズ=メモリのみ（新キーなし）・**Date+11分偽装→30秒interval経由で再発火を実証**・
>   再発火時はボタン非表示=1回制限が機能・日付変更でスヌーズもリセット・再発火前に
>   remind/時刻変更/完了/スキップを再チェックする防御的実装。合格
> - **確認済みの安全点**: set-reminder-sound要素はindex.htmlに存在（applySettingsが?.なしで
>   触るため要素欠落なら起動停止だったが問題なし）／新localStorageキーなし／コンソールエラーゼロ
> - **既知の軽微な仕様**: 23:50頃のスヌーズは日付変更リセットで消える（リロードで消えるのと
>   同等の割り切り。リマインダー自体がタブ起動中のみの仕組みのため許容）
