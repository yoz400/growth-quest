...
- [ ] どの瞬間も「操作できるオーバーレイ」は最前面の1つだけ
- [ ] 非表示のオーバーレイは Tabキーでフォーカスできない
- [ ] 非表示のオーバーレイはスクリーンリーダーに読まれない（`aria-hidden` が正しく切り替わる）
- [ ] ESCキーは「最前面のオーバーレイ」だけを閉じる（1か所の共通処理）
- [ ] オーバーレイを閉じたら、開く前にフォーカスしていた要素にフォーカスが戻る
- [ ] 既存の見た目・アニメーション（フェードイン等）は変わらない
- [ ] オンボーディング（召喚→妖精ガイド）の流れが今まで通り動く
- [ ] すごろく中の「装備ゲット」のような **重ね表示** も動く（§3.2 スタック方式）
