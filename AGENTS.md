...
1. `docs/AI/CURRENT.md` — 今プロジェクトがどういう状態か
2. 指定された `docs/spec_*.md` — 発注書
3. `docs/AI/FAILURES.md` — 同種の作業で過去に事故がないか
...
受け入れ基準を満たしていない項目があれば、**完了と報告しない。**
「たぶん動く」「動くはず」で報告しない — 実際に動かして確認する。

## docs/AI/ への書き込み

`docs/AI/` 配下は**編集しない。読むだけ。**
記録（CURRENT / DECISIONS / FAILURES の更新）はクロが行う。
残したいことがあれば、完了報告の Decisions / Problems に書けばよい。

理由: 書き手が複数いると、上書き・書式のばらつき・
「どちらが最新か分からない」状態が必ず起きるため、書き手を1人に固定している。
